...
public class SearchResultsPage {
 ...
 public static WebElement searchBtn = hooks.MyHooks.wait.until(ExpectedConditions.visibilityOfElementLocated(name));
...
public static void addFirstProduct(){
     WebElement addToCart = hooks.MyHooks.wait.until(ExpectedConditions
                .visibilityOfElementLocated(addPurchase));
        Elements.clickOnlyIfElementPresent(addToCart);
        WebElement addItemToTheCart = hooks.MyHooks.wait.until(ExpectedConditions
...
public static void addOptionsOfOutOfStockProduct() throws InterruptedException, AWTException {
    WebElement radioBtn = hooks.MyHooks.wait.until(ExpectedConditions
            .elementToBeClickable(radio));
    Elements.clickOnlyIfElementPresent(radioBtn);
    WebElement checkBox = hooks.MyHooks.wait.until(ExpectedConditions
            .elementToBeClickable(check));
    Elements.clickOnlyIfElementPresent(checkBox);
    WebElement textField = hooks.MyHooks.wait.until(ExpectedConditions
            .elementToBeClickable(text));
    Elements.TypeText(textField,"Test information");
    WebElement selectField = hooks.MyHooks.wait.until(ExpectedConditions
            .elementToBeClickable(select));
    Select select1 = new Select(selectField);
    select1.selectByIndex(3);
    WebElement enterTextField = hooks.MyHooks.wait.until(ExpectedConditions
    ...
    WebElement fileLoadBtn = hooks.MyHooks.wait.until(ExpectedConditions
...
//=== create path to the file on the computer
    ...
    hooks.MyHooks.wait.until(ExpectedConditions.alertIsPresent());
    Alert alert = hooks.MyHooks.driver.switchTo().alert();
    alert.accept();
